...
test: tee/private.pem
	...
	@docker run --user root -e TWITTER_TEST_ACCOUNT -v $(PWD)/coverage:/app/coverage --rm --workdir /app $(IMAGE) go test -coverprofile=coverage/coverage.txt -covermode=atomic -v ./...

...
test: tee/private.pem $(TEST_COOKIE_DIR)
	...
	@docker run --user root -e TWITTER_TEST_ACCOUNT -e TEST_COOKIE_DIR=/cookies -v $(TEST_COOKIE_DIR):/cookies -v ./.env:/home/masa/.env -v $(PWD)/coverage:/app/coverage --rm --workdir /app $(IMAGE) go test -coverprofile=coverage/coverage.txt -covermode=atomic -v ./...

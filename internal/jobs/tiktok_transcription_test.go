...
var _ = Describe("TikTokTranscriber", func() {
	...
	Context("when arguments are invalid", func() {
		It("should return an error if VideoURL is empty but not record error stats", func() {
			jobArguments := map[string]interface{}{
				"video_url": "", // Empty URL
			}
			...
			Eventually(func() uint {
				...
				return workerStatsMap[stats.TikTokTranscriptionErrors]
			}, 5*time.Second, 100*time.Millisecond).Should(BeNumerically("==", 0), "TikTokTranscriptionErrors count should be 0 for user validation errors")
